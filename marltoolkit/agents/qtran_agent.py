...
class QTranAgent(object):
    ...
    def learn(
        ...
        **kwargs
        ...
        actions_onehot_batch = actions_onehot_batch.to(self.device, dtype=torch.long)
        ...
        local_hidden_states = local_hidden_states.reshape(
            batch_size, self.n_agents, episode_len, -1
        ).transpose(
            1, 2
        )  # btav

        target_local_hidden_states = torch.stack(target_local_hidden_states, dim=1)
        ...
        td_targets = (
            ...
            + self.gamma * (1 - terminated_batch.reshape(-1, 1)) * target_joint_qs
        )
        #  Td-error
        ...
        nopt_values = (
            chosen_action_local_qs.sum(dim=2).reshape(-1, 1) - joint_qs.detach() + vs
        )
        ...
        loss = (
            td_loss
            + self.opt_loss_coef * opt_loss
            + self.nopt_min_loss_coef * nopt_loss
        )
